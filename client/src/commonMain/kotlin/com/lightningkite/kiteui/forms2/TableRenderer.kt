 ...
 * @param items Reactive list of items to display
...
fun <T> ViewWriter.renderTable(
    ...
    columns: MutableReactive<List<DataClassPath<T, *>>>,
    linkTo: ((T) -> () -> Page)? = null,
    ...
    scrollingHorizontally.col {
        ...
        expanding.changingSizeConstraints {
            ...
            expanding.themed(ListSemantic).recyclerView {
                // Handle LimitReadable pagination
                reactive {
                    val itemsValue = items
                    if (itemsValue is LimitReadable<T>) {
                        if (itemsValue.limit < lastIndex() + 50) {
                            itemsValue.limit = lastIndex() + 100
                        }
                    }
                }

                children(items, id = { (it as? HasId<*>)?._id ?: it }) { itemReactive ->
