...
object DataClassRenderer : Renderer<Any> {
    ...
    override fun form(context: RenderContext<Any>, value: MutableReactive<Any>, module: FormModule): ViewWriter.() -> Unit {
        ...
        val visibleProps = properties
            .filter { it.fieldVisibility != FieldVisibility.HIDDEN }
            ...
            col {
                for (group in grouped) {
                    if (group.size == 1) {
                        ...
                        if (prop.fieldVisibility == FieldVisibility.EDIT) {
...
val SerializableProperty<*, *>.fieldVisibility: FieldVisibility
    get() {
        for (ann in serializableAnnotations) {
            when (ann.fqn) {
                Annotations.AdminHidden -> return FieldVisibility.HIDDEN
                Annotations.AdminViewOnly -> return FieldVisibility.READ
                Annotations.Denormalized -> return FieldVisibility.READ
            }
        }
        // Auto-hide UUID _id fields
        if (name == "_id" && serializer.descriptor.serialName.contains("Uuid")) {
            if (serializableAnnotations.none { it.fqn == Annotations.References || it.fqn == Annotations.MultipleReferences }) {
                return FieldVisibility.HIDDEN
            }
        }
        return FieldVisibility.EDIT
    }
