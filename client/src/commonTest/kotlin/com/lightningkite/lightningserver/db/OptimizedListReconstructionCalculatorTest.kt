...
/**
 * Tests for OptimizedListReconstructionCalculator to ensure it maintains
 * the same behavior as NaiveListReconstructionCalculator
 */
class OptimizedListReconstructionCalculatorTest {
    ...
    fun basic() = runTest2 {
        ...
        val cache = OptimizedListReconstructionCalculator(LargeTestModel.serializer(), clock = Clock.default())
    ...
    @Test fun multipleQueriesShareItems() = runTest2 {
        val cache = OptimizedListReconstructionCalculator(LargeTestModel.serializer(), clock = Clock.default())
        ...
        // Two queries with different conditions
        val query1 = Query<LargeTestModel>(
            condition { it.int lt 3 },
            orderBy = sort { it.int.ascending() }
        )
        val query2 = Query<LargeTestModel>(
            condition { it.int gt 2 },
            orderBy = sort { it.int.ascending() }
        )

        // Add initial data for both queries
        cache.update(CacheUpdate.QueryResult(query1, listOf(model1, model2)))
        cache.update(CacheUpdate.QueryResult(query2, listOf(model3, model4)))

        // Verify initial state
        assertEquals(listOf(1, 2), cache.cached(query1)!!.item.map { it.int })
        assertEquals(listOf(3, 4), cache.cached(query2)!!.item.map { it.int })

        // Update model2 to have int = 20 (should move from query1 to query2)
        val updatedModel2 = model2.copy(int = 20)
        cache.update(CacheUpdate.MutationResult(listOf(updatedModel2)))

        // Verify query1 no longer includes model2
        val result1 = cache.cached(query1)!!.item
        assertEquals(listOf(1), result1.map { it.int })

        // Verify query2 now includes the updated model2
        val result2 = cache.cached(query2)!!.item
        assertEquals(3, result2.size)
        assertEquals(setOf(3, 4, 20), result2.map { it.int }.toSet())
    }
}
